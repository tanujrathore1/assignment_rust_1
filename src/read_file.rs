...
pub fn read_txt_file(file_path:String, id: i32, temp :&mut HashMap<String,Employee>) -> Result<(),io::Error>{
    ...
    for entries in content.lines() {
        ...
        // println!("{:?}",values);
        let emp_id = values.next().expect("No emp id").parse::<i32>().unwrap();
        let emp_name_data = values.next().expect("No emp name").to_string();
        let dept_id = values.next().expect("No emp name").parse::<i32>().unwrap();
        let mobile_no = values.next().expect("No emp name").to_string();
        let email = values.next().expect("No emp name").to_string();
...
pub fn read_dept_file_data(file_path:String, id : i32, temp :&mut HashMap<String,Dept>, 
    file_name :&str) -> Result<(),io::Error>{
...
pub fn read_salary_data(file_path:String, id : i32, temp :&mut HashMap<String,Salary>, 
    file_name :&str) -> Result<(),io::Error>{
    let mut excel = Excel::open(file_path).unwrap();
    let r = excel.worksheet_range("Sheet1").unwrap();
...
fn date_from_float(dte:f64)->NaiveDate {
    ...
    let date = start.checked_add_signed(Duration::days((dte-2.0).to_string().parse::<i64>().unwrap()));

    date.unwrap()

}

pub fn read_leave_data(file_path:String, id : i32, temp :&mut HashMap<String,Leave>, 
    file_name :&str) -> Result<(),io::Error>{
    let mut excel = Excel::open(file_path).unwrap();
    let r = excel.worksheet_range("Sheet1").unwrap();
    ...
    for row in r.rows() {
        ...
        if file_name == "leave"{
            ...
            if emp_id == id{
                ...
                // println!("leave month to: {:?}", leave_month_to);
                // println!("leave day: {:?}", leave_day);
                // println!("leave month from: {:?}", leave_month_from);
                // println!("current: {:?}", current_month);
                ...
                // else{
                //     leave_count = leave_month as i32;
                // }
